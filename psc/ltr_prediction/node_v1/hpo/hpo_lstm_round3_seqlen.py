#!/usr/bin/env python3
"""
HPO Round 3 — LSTM+NODE, longer sequences.

Fixed: architecture from trial 37 (round 1), best training params from round 2.
Explores: seq_len in [150, 200, 250, 300, 400].

build_dataset is reimplemented here to accept variable seq_len.
"""
...
# Fixed architecture from round 1 trial 37
HIDDEN_SIZE = 128
NUM_LAYERS = 1
FC_HIDDEN = 64
PHYSICS_PROJ_DIM = 16

# Fixed training params from round 2 best (trial 10)
ACTIVATION = 'gelu'
PROJ_DEPTH = 2
USE_ADAMW = False
SCHED_PATIENCE = 5

SEQ_LENS = [150, 200, 250, 300, 400]


# ============== Variable-length build_dataset ==============

def build_dataset_variable(files, horizon_steps, seq_len, stride=10):
    """Like train_with_node.build_dataset but with configurable seq_len."""
    X_all, Y_all = [], []
    for f in files:
        try:
            df = load_scenario(f)
...
class PhysicsProjection_R3(nn.Module):
    def __init__(self, node_backbone, backbone_dim=128, out_dim=16, activation='gelu'):
        super().__init__()
        self.node_backbone = node_backbone
        act_fn = ACTIVATIONS[activation]
        # depth=2 (best from round 2)
...
class LSTM_Physics_R3(nn.Module):
...
def load_shared_resources():
    ...
    # Build datasets for each seq_len
    datasets = {}
    for seq_len in SEQ_LENS:
        ...
        if len(X_train) == 0 or len(X_test) == 0:
            print(f"  seq_len={seq_len}: SKIPPED (not enough data)")
            continue
...
def objective(trial, shared):
    ...
    lr_other = trial.suggest_float('lr_other', 3e-4, 1e-3, log=True)
    ...
    try:
        physics_proj = PhysicsProjection_R3(
            shared['backbone'], shared['backbone_dim'],
            out_dim=PHYSICS_PROJ_DIM, activation=ACTIVATION,
        ).to(device)
        for p in physics_proj.node_backbone.parameters():
            p.requires_grad = False

        model = LSTM_Physics_R3(
...
def retrain_best(best_params, shared):
    print(f"\n  Retraining best round 3 config (seq_len={best_params['seq_len']})...")
...
def main():
    t_start = time.time()
    print("=" * 70)
    print("HPO Round 3 — LSTM+NODE (longer sequences)")
    print(f"Device: {device}")
    print(f"Seq lens: {SEQ_LENS}")
    print(f"Fixed: arch=trial37, activation={ACTIVATION}, proj_depth=2, Adam")
    print(f"Exploring: seq_len, weight_decay, lr, dropout, batch_size")
    print("=" * 70)
    ...
    study = optuna.create_study(
        study_name="hpo_LSTM_NODE_D2_h2s_round3_seqlen",
        storage=storage,
        direction='maximize',
    ...
    save_path = MODEL_DIR / f"hpo_LSTM_{HORIZON_KEY}_{CONFIG}_r3_seqlen.pt"
    ...
    results = {
        ...
        'round2_best_auc': 0.887,
    }

    results_path = OUTPUT_DIR / "results_hpo_lstm_round3_seqlen.json"
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)

    elapsed = time.time() - t_start
    print(f"\nRound 3 done in {elapsed/60:.1f} min")
    print(f"R1 best: 0.887 | R2 best: 0.887 → R3 best: {final_auc:.4f}")
    print("DONE!")
