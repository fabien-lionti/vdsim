#!/usr/bin/env python3
"""
HPO PatchTST Round 3 — longer sequences.

Fixed: best PatchTST architecture from round 1 (trial 1).
Explores: seq_len in [150, 200, 250, 300, 400], patch_size, lr, dropout.
"""
...
# Fixed from round 1 best PatchTST (trial 1)
PHYSICS_PROJ_DIM = 48
...
class PhysicsProjection(nn.Module):
    def __init__(self, node_backbone, backbone_dim=128, out_dim=48):
        super().__init__()
...
class PatchTST_Physics(nn.Module):
    def __init__(self, physics_proj, seq_len, n_features, patch_size=10,
                 d_model=128, nhead=2, num_layers=3, dim_feedforward=256,
                 output_size=3):
        super().__init__()
...
def objective(trial, shared):
    ...
    # patch_size must divide seq_len — offer valid choices
    valid_patches = [p for p in [5, 10, 15, 20, 25] if seq_len % p == 0]
    if not valid_patches:
        valid_patches = [10]  # fallback, will truncate
    patch_size = trial.suggest_categorical('patch_size', valid_patches)
    ...
    lr_other = trial.suggest_float('lr_other', 5e-5, 5e-4, log=True)
...
def main():
    t_start = time.time()
    print("=" * 70)
    print("HPO PatchTST — longer sequences")
    print(f"Device: {device}")
    print(f"Seq lens: {SEQ_LENS}")
    print(f"Fixed: physics_proj_dim={PHYSICS_PROJ_DIM}")
    print(f"Exploring: seq_len, patch_size, d_model, nhead, num_layers, lr, etc.")
    print("=" * 70)
    ...
    study = optuna.create_study(
        study_name="hpo_PatchTST_NODE_D2_h2s_round3_seqlen",
        storage=storage,
        direction='maximize',
        sampler=TPESampler(seed=789),
    ...
    save_path = MODEL_DIR / f"hpo_PatchTST_{HORIZON_KEY}_{CONFIG}_r3_seqlen.pt"
    ...
    results = {
        ...
        'round1_best_auc': 0.851,
    }

    results_path = OUTPUT_DIR / "results_hpo_patchtst_round3_seqlen.json"
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)

    elapsed = time.time() - t_start
    print(f"\nPatchTST round 3 done in {elapsed/60:.1f} min")
    print(f"R1 best: 0.851 → R3 best: {final_auc:.4f}")
    print("DONE!")
